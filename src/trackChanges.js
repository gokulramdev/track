...
export function amendTransaction(tr, state, user) {

    console.table("New_track", [
        tr.steps.length,
        tr.meta,
    ...
    if (
        ...
        return trackedTransaction(
            ...
            state,
            user,
            // !false && !["write-tracked", "review-tracked"].includes('write'),
            false,
            Date.now() - 0
    ...
    tr.steps.forEach((originalStep, originalStepIndex) => {
        ...
        if (step instanceof ReplaceStep) {
            ...
            doc.nodesBetween(step.from, step.to, (node, pos) => {
                ...
                if (
                    ["bold", "italic", "underline"].includes(step.mark.type.name) &&
            ...
            doc.nodesBetween(step.from, step.to, (node, pos) => {
                ...
                console.log("includes", (step.mark))
                if (
                    ["bold", "italic", "underline"].includes(step.mark.type.name) &&
                    node.marks.find(mark => mark.type === step.mark.type)
                ) {
